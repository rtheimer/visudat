...
## Remote
```ssh -p 41023 solar.orca-grp.de```

- user: &nbsp;  visudat
- passwd:   &nbsp; visuadmin
## Webproxy NGINX
### sites-enabled :  &nbsp;&nbsp; solar.orca-grp.de
- ### test nginx config
```
nginx -t
```
...
server {
        listen 80;
        server_name solar.orca-grp.de;


location /static/ {
        root /home/visudat/production/static/;
}
...
## WSGI Gunicorn
```
command = '/home/visudat/.pyenv/version/django-api/bin/gunicorn'
pythonpath = '/home/visudat/production/visudat'
bind = '127.0.0.1:8000'
worker = 3
``````
- ### start gunicorn
gunicorn &nbsp; -c &nbsp; conf/gunicorn_config.py visudat.wsgi
...
#
<VirtualHost solar.orca-grp.de>
#ServerAdmin             ftpmaster@server.com
#ServerName              "Big FTP Archive"
TransferLog             /var/log/proftpd/xfer/ftp.solar.server
#MaxLoginAttempts        3
MasqueradeAddress      80.153.199.139
RequireValidShell       no
AuthUserFile  /etc/proftpd/ftpd.passwd
DefaultRoot ~
AllowOverwrite          yes
#DefaultRoot             /var/www/upload
PassivePorts 49900  49999
AllowOverwrite          yes
</VirtualHost>
...
DefaultRoot ~
# Handles the AuthUserFile and AuthGroupFile directives, for storing user account information
